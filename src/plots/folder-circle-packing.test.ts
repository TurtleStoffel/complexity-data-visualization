...
import {
    getFolders,
    prepareFolderCirclePackingData,
} from "./folder-circle-packing";

import { FileMetrics } from "../data";

test("getFolder should strip filename from path", () => {
    const path = "src/plots/filename.ts";
    expect(getFolders(path)).toEqual(
        expect.arrayContaining(["src/plots", "src", "root"]),
    );
});
